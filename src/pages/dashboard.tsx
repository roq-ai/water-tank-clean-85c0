...
import { getRepresentatives } from 'apiSdk/representatives';
...
function DashboardPage() {
  ...
  const representativeStat = useSWR<PaginatedInterface<RepresentativeInterface>>(
    () => `/representatives`,
    () => getRepresentatives({ limit: 1, offset: 0 }),
  );

  const serviceStat = useSWR<PaginatedInterface<ServiceInterface>>(
    () => `/services`,
    () => getServices({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: representativeStat?.data?.totalCount,
      isLoading: representativeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Services',
      result: serviceStat?.data?.totalCount,
      isLoading: serviceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
