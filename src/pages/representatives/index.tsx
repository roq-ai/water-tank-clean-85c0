...
import { getRepresentatives, deleteRepresentativeById } from 'apiSdk/representatives';
import { RepresentativeInterface } from 'interfaces/representative';

type ColumnType = ColumnDef<RepresentativeInterface, unknown>;

interface RepresentativeListPageProps {
...
export function RepresentativeListPage(props: RepresentativeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RepresentativeInterface>>(
    () => `/representatives?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteRepresentativeById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: RepresentativeInterface) => {
    if (hasAccess('representative', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/representatives/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/representatives/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('representative', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/representatives/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('representative', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="representative" />
        </Flex>

        {hasAccess('representative', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/representatives/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
