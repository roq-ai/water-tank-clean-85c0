...
import { getRepresentativeById, updateRepresentativeById } from 'apiSdk/representatives';
...
function RepresentativeEditPage() {
  ...
  const handleSubmit = async (values: RepresentativeInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateRepresentativeById(id, values);
      mutate(updated);
      resetForm();
