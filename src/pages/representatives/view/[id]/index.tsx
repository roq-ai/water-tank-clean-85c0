...
import { getRepresentativeById } from 'apiSdk/representatives';
import { RepresentativeInterface } from 'interfaces/representative';

function RepresentativeViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<RepresentativeInterface>(
    () => (id ? `/representatives/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('representative', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/representatives/edit/${id}`} passHref legacyBehavior>
                    <Button
